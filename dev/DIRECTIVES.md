# iqdb-ivf -- Engineering Directives
...
iqdb-ivf partitions the vector space into clusters and searches only the most relevant ones. It is the complement to HNSW: more memory-efficient at very large scale and with more predictable latency. It ships IVF-Flat first; IVF-PQ (quantized within clusters) layers on `iqdb-quantize`.
...
- IVF requires training on a representative sample before use; this UX cliff is documented loudly.
- Probe count is the recall/latency dial and is exposed and tested.
...
- `iqdb-filter`: filtered cluster search
- `iqdb-quantize`: optional, for IVF-PQ
